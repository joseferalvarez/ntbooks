...
export default function Book({ book, setCurrentBook }: { book: IBook; setCurrentBook: (book: IBook) => void }) {
  return (
    <div
      className="flex flex-col p-[15px] shadow-md rounded bg-white min-h-[450px] gap-[10px]"
    ...
    >
      ...
      <div className="grow flex flex-col gap-[5px]">
        ...
        <p className="book-author">
          {book.author_name && book.author_name.length > 1 && book.author_name.map((author, index) => 
            (index < book.author_name.length - 1 ? (`${author} & `) : (author))
          )}
